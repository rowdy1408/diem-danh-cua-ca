document.addEventListener('DOMContentLoaded', () => {
    ...
    // (Lấy từ Project Settings -> Your Apps -> Chọn App -> SDK setup -> CDN)
    // =================================================================
    ...
    (()=>{
        ...
        copyReportBtn.addEventListener('click', async () => {
            ...
            window.showModal('Xem Trước Báo Cáo', previewContent, 'Sao chép nội dung', 'Đóng', () => {
                navigator.clipboard.writeText(reportText).then(() => { alert('Đã sao chép báo cáo vào bộ nhớ đệm!'); document.getElementById('confirmation-modal').classList.add('hidden'); });
            });
        });
        ...
        const parseSchedule = (code, text) => { const standardMap = { 'T2': 1, 'T3': 2, 'T4': 3, 'T5': 4, 'T6': 5, 'T7': 6, 'CN': 0 }; if (code !== 'Khac') { const days = code.split('-'); if (days.every(day => day in standardMap)) { return days.map(day => standardMap[day]); } } if (code === 'Khac' && text) { const customMap = { '2': 1, 'hai': 1, '3': 2, 'ba': 2, '4': 3, 'tư': 3, '5': 4, 'năm': 4, '6': 5, 'sáu': 5, '7': 6, 'bảy': 6, 'cn': 0, 'nhật': 0 }; const matches = text.toLowerCase().match(/hai|ba|tư|năm|sáu|bảy|nhật|cn|[2-7]/g); if (!matches) return []; const dayNumbers = matches.map(match => customMap[match]); return [...new Set(dayNumbers)]; } return []; };
