document.addEventListener('DOMContentLoaded', () => {
  ...
  appId: "1:38123679904:web:b27668ed199dc16f43d9c5"
    ...
    (()=>{
        ...
        const parseSchedule = (code, text) => { const standardMap = { 'T2': 1, 'T3': 2, 'T4': 3, 'T5': 4, 'T6': 5, 'T7': 6, 'CN': 0 }; if (code !== 'Khac') { const days = code.split('-'); if (days.every(day => day in standardMap)) { return days.map(day => standardMap[day]); } } if (code === 'Khac' && text) { const customMap = { '2': 1, '3': 2, '4': 3, '5': 4, '6': 5, '7': 6, 'cn': 0 }; const days = text.replace(/thứ/gi, '').toLowerCase().split('-'); return days.map(day => customMap[day.trim()]).filter(dayNum => dayNum !== undefined); } return []; };
