document.addEventListener('DOMContentLoaded', () => {
  ...
  appId: "1:38123679904:web:b27668ed199dc16f43d9c5"
    ...
    (()=>{
        ...
        const CORRECT_PASSWORD = '30082025@NN';
        ...
        function handleFailedAttempt() {
            ...
            if (remainingAttempts > 0) {
                errorMessage.textContent = `Sai rồi, nhập lại thôi. Còn ${remainingAttempts} lần nữa thôi.`;
                localStorage.setItem('loginAttempts', attempts);
            } else {
                errorMessage.textContent = `Bạn đã nhập sai ${MAX_ATTEMPTS} lần.`;
                lockLogin();
            }
        }
        ...
        function startLockoutTimer(lockoutUntil) {
            const intervalId = setInterval(() => {
                ...
                if (remainingTime <= 0) {
                    ...
                    enableInputs();
                    errorMessage.textContent = `Đã hết thời gian khóa. Bạn có thể thử lại.`;
                } else {
                    const minutes = Math.floor((remainingTime / 1000) / 60);
                    const seconds = Math.floor((remainingTime / 1000) % 60);
                    errorMessage.textContent = `Vui lòng thử lại sau ${minutes} phút ${seconds} giây.`;
                }
            }, 1000);
        }
    ...
    (()=>{
        ...
        const showModal = (title, contentHTML, confirmText, cancelText, onConfirm) => {
        ...
        classScheduleSelect.addEventListener('change', () => {
            customScheduleGroup.classList.toggle('hidden', classScheduleSelect.value !== 'Khac');
        });
        ...
        newClassForm.addEventListener('submit', (e) => {
            ...
            if (classScheduleSelect.value === 'Khac' && !scheduleText.toLowerCase().includes('thứ')) {
                alert('Lịch dạy tùy chỉnh phải chứa chữ "Thứ".');
                return;
            }
    ...
    (()=>{
        ...
        const parseSchedule = (code, text) => {
            ...
            if (code === 'Khac' && text) {
                const customMap = { '2': 1, '3': 2, '4': 3, '5': 4, '6': 5, '7': 6, 'cn': 0 };
                const days = text.replace(/thứ/gi, '').toLowerCase().split('-');
                return days.map(day => customMap[day.trim()]).filter(dayNum => dayNum !== undefined);
            }
            return [];
        };
        ...
        const showAttendanceSheet = (classData) => {
            currentAttendanceClass = classData;
            currentDisplayDate = new Date();
            document.getElementById('attendance-header').textContent = `Điểm danh lớp: ${classData.studentName}`;
            renderCalendar();
        ...
        calendarGrid.addEventListener('click', (e) => {
            ...
            if (!allAttendance[currentAttendanceClass.id]) {
                allAttendance[currentAttendanceClass.id] = {};
            }
        ...
        copyReportBtn.addEventListener('click', () => {
            ...
            window.showModal('Xem Trước Báo Cáo', previewContent, 'Sao chép nội dung', 'Đóng', () => {
                navigator.clipboard.writeText(reportText).then(() => {
                    alert('Đã sao chép báo cáo vào bộ nhớ đệm!');
    ...
    (()=>{
        ...
        btnSalaryReport.addEventListener('click', () => {
            const now = new Date();
            monthPicker.value = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
            renderReport();
            showView(salaryReportView);
        });
        monthPicker.addEventListener('change', renderReport);
        
        reportContent.addEventListener('click', (e) => {
            if (e.target.classList.contains('hard-delete-btn')) {
                handleHardDelete(e.target.dataset.id);
            }
        });

        const handleHardDelete = (classId) => {
            const classes = getArrayFromStorage('classes');
            const classToDelete = classes.find(c => c.id === classId);
            if (!classToDelete) return;
            if (confirm(`HÀNH ĐỘNG NÀY KHÔNG THỂ HOÀN TÁC!\n\nBạn có chắc chắn muốn XÓA VĨNH VIỄN lớp của học viên "${classToDelete.studentName}" và TOÀN BỘ dữ liệu điểm danh của nó không?`)) {
                const updatedClasses = classes.filter(c => c.id !== classId);
                saveToStorage('classes', updatedClasses);
                const allAttendance = getObjectFromStorage('attendance');
                delete allAttendance[classId];
                saveToStorage('attendance', allAttendance);
        ...
        function renderReport() {
            ...
            classes.forEach(cls => {
                ...
                if (presentCount > 0 || cls.isActive === false) {
